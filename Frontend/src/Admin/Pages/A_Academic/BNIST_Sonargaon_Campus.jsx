...
const BNIST_Sonargaon_Campus = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${API_BASE_URL}/fetchSonargaon`);
        ...
        if (data.image) {
          ...
          setImagePreview(`${API_BASE_URL}/storage/photos/shares/Neded%20Picture/1545365825sonargaon.jpg`);
        }
      } catch (error) {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const response = await authFetch(`${API_BASE_URL}/saveSonargaon`, {
        method: "POST",
        body: formData,
      });
      if (response.ok) {
          ...
          <div className="w-full">
            ...
            <div className="p-8 md:p-14">
              <div className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                <div>
                   <h2 className="text-4xl font-black text-slate-900 mb-3 tracking-tight">Sonargaon Campus</h2>
...
export default BNIST_Sonargaon_Campus;
