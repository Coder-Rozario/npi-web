...
import { MdOutlineDashboard, MdOutlineSchool, MdTimer, MdAccountBalanceWallet, MdPersonOutline, MdVerified, MdLayers } from "react-icons/md";
import { FaBookOpen, FaArrowRight, FaFacebookMessenger } from "react-icons/fa";
import { Link } from "react-router-dom";
import Loading from "../../Components/Loading/Loading";
import { motion, AnimatePresence } from "framer-motion";

const DepartmentLayout = ({ apiUrl, departmentTitle, heroAlt = "Department Hero" }) => {
  ...
  const sanitizeAndOptimize = (html) => {
    ...
    return safe
      ...
      .replace(/src=(['"])uploads\//g, `src=$1${API_BASE_URL}/uploads/`)
  ...
  return (
    <div className="bg-[#F8FAFC] min-h-screen font-sans text-slate-900 overflow-x-hidden">
      <div className="relative h-[40vh] lg:h-[70vh] w-full flex items-center lg:items-start lg:pt-16 overflow-hidden">
        ...
        <div className="mx-auto lg:w-[75vw] px-2 sm:px-3 md:px-6 relative z-10 text-left">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-4xl mx-auto lg:mx-0">
            ...
            <h1 className="text-3xl md:text-6xl font-black text-white mb-4 tracking-tighter">
      ...
      <div className="mx-auto lg:w-[75vw] px-0 lg:px-6 -mt-10 lg:-mt-24 relative z-20 pb-20">
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="w-full text-justify text-sm lg:w-[65%]">
                ...
                {["overview", "curriculum"].map((tab) => (
                  <button
                    ...
                    className={`relative flex-1 flex items-center justify-center gap-2 lg:gap-3 py-4 lg:py-5 rounded-none lg:rounded-2xl font-bold text-xs lg:text-sm uppercase tracking-wider transition-all duration-300 ${
                      activeTab === tab ? "bg-white shadow-sm lg:shadow-md" : "text-slate-400 hover:text-slate-600"
                    }`}
                    style={activeTab === tab ? { color: '#0186C0' } : {}}
                  >
                    {tab === "overview" ? <MdOutlineDashboard size={18} /> : <FaBookOpen size={16} />}
                    {tab}
                    {activeTab === tab && <motion.div layoutId="activeTab" className="absolute bottom-2 w-6 h-1 rounded-full" style={{ backgroundColor: '#0186C0' }} />}
                  </button>
                ))}
              </div>

              <div className="px-6 py-8 lg:p-14 text-justify">
                <style>{`
                  .dynamic-dept-content {
                    line-height: 1.8;
                    color: #475569;
                    font-size: 1.05rem;
                  }
                  .dynamic-dept-content h1,
                  .dynamic-dept-content h2,
                  .dynamic-dept-content h3 {
                    color: #0f172a;
                    font-weight: 900;
                    margin-top: 2.5rem;
                  ...
                  .dynamic-dept-content h1 { font-size: 2.25rem; }
                  .dynamic-dept-content h2 { font-size: 1.85rem; }
                  .dynamic-dept-content h3 { font-size: 1.5rem; }
                  .dynamic-dept-content p {
                    margin-bottom: 1.5rem;
                    text-align: justify;
                  }
                  .dynamic-dept-content b,
                  .dynamic-dept-content strong {
                    color: #1e293b;
                    font-weight: 700;
                  }
                  .dynamic-dept-content table {
                    width: 100%;
                ...
                <AnimatePresence mode="wait">
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -10 }}
                    className="dynamic-dept-content"
                  >
                    <div
                      dangerouslySetInnerHTML={{
                        __html: sanitizeAndOptimize(activeTab === "overview" ? data?.overview : data?.curriculum),
                      }}
                    />
                  </motion.div>
                </AnimatePresence>
